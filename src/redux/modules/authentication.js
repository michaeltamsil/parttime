...
import {
  ...
  handleError
} from '../../helpers/store-utils';
import {
  getAppUrl
} from '../../helpers/environment-utils';

const AUTH_ENDPOINT_BASE = 'api';
...
export const login = (credentials, desiredPath) => async (dispatch) => {
  try {
    ...
    if (response) {
      setCookie('token', 'JWT '+response.user.token, { id: response.user.id });
  ...
  try {
    ...
    if (response) {
      setCookie('token', response.token, {
        maxAge: response.tokenExpiration
      });
  ...
  switch (action.type) {
    case CHANGE_AUTH:
      return updateStore(state, action, {
        authenticated: Boolean(_.get(action, 'payload')),
        status: Boolean(_.get(action, 'payload.message')),
        user: _.get(action, 'payload.id')
      });
    case GET_AUTHENTICATED_USER:
    console.log('masuk sini');
      return updateStore(state, action, {
        user: _.get(action, 'payload')
      });
    default:
      return state;
  }
